...
if (rank==0) :
    ...
    for iProc in range(1,nbp):
        xbeg+=Nloc
        xend+=Nloc
        comGlobal.send([xbeg,xend], iProc)


    stat:MPI.Status=MPI.Status()
    done=comGlobal.recv(status=stat)

else:
    status:MPI.Status=MPI.Status()
    res:int=1
    [xbeg,xend]=comGlobal.recv(source=0)
    ConvergenceLoc =np.empty((abs(xend-xbeg), height), dtype=np.double)
    ConvergenceLoc=calcLocalconvergence(xbeg,xend,width)
    req:MPI.Request=comGlobal.isend(res,0)
...
comGlobal.Scatter(convergence,ConvergenceLoc, root=0)
