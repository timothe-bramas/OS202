...
void CartesianGridOfSpeed::updateVelocityField(Simulation::Vortices const &t_vortices)
{
    using point = Simulation::Vortices::point;
    double halfStep = 0.5 * m_step;
    std::size_t iRow, jCol;
#pragma omp parallel for private(iRow, jCol) shared(t_vortices) num_threads(4)

    for (iRow = 0; iRow < m_height; ++iRow)
    {
        double yP = m_bottom + iRow * m_step + halfStep;
        for (jCol = 0; jCol < m_width; ++jCol)
        {
