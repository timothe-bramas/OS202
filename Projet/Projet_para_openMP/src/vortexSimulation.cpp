...
auto readConfigFile( std::ifstream& input , int nbProcessors, int sliceNo)
{
    ...
    else 
    {
        ...
        // Calculate the width
        double width = static_cast<double> (nx) * h;

        // Divide according to processor number
        nbPoints = nbPoints / nbProcessors;

        // Calculate the bottom-left bound
        xl = -50 + width * sliceNo / nbProcessors;
        yb = -50;

        // Calculate the top-right bound
        xr = -50 + width * (sliceNo + 1) / nbProcessors;
        yt = 50;
