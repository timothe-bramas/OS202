...
int main( int nargs, char* argv[] )
{
    ...
    if(nbp!2){std::cout<<"There must have 2 processes on this program";return EXIT_FAILURE;}
    ...
    double dt = 0.1;
    double advance=0;
    double keyboard[4]={animate,advance,again,dt}
    ...
    while (myScreen.isOpen())
    {
        ...
        while (myScreen.pollEvent(event))
        {
            ...
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::P)) animate = 1;
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::S)) animate = 0;
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up)) dt *= 2;
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down)) dt /= 2;
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right)) advance = 1;
            tab[0]=animate;
            tab[1]=advance;
            tab[3]=dt;

        }
        MPI_Send(tab,4,MPI_DOUBLE,1,1234,globcom);
    ...
    if(rank==1){
        ...
        double*tabloc=None;
        while(again==1){
            MPI_recv(tabloc,4,MPI_DOUBLE,0,1234,globcom);
